...
BOOL XXXXXXXDlg::PreTranslateMessage(MSG* pMsg)
{
	...
	else if (pMsg->message == WM_LBUTTONDOWN)//按下鼠标的消息
	{
		GetCursorPos(&MousePoint);//获取鼠标的位置 MousePoint的类型为CPoint
		//HWND hWnd = (HWND)cvGetWindowHandle("color");		
		//::ScreenToClient(hWnd, &MousePoint);//获取相对于彩色图矩形的鼠标坐标
	}
